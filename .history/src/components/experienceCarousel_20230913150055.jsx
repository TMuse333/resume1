...
function shiftLeft(elementIds) {
  ...
  const prevElement = parseInt(elements[elementIds.length - 1].style.left);
  const currentElement = parseInt(elements[0].style.left);
  const distance = currentElement - prevElement - 50;

  for (let i = 0; i < dimensions.length; i++) {

      elements[i].style.transform = `translateX(${distance[i]}px)`;
