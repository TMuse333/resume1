...
const ExperienceCarousel = () => {
  ...
  function shiftElementsToCenter2(elementIds) {
    const elements = elementIds.map((elementId) => document.getElementById(elementId));
  
    // Initialize centerLeft and top based on the first element's position
    let centerLeft = null;
    let top = null;
    ...
    elements.forEach((element, index) => {
      if (element) {
        ...
        // Calculate the new left position to center the element
        if (index === 0) {
          // For the first element, calculate initial center position
        ...
        element.style.left = centerLeft - elementWidth / 2 + 'px';
        element.style.top = top - elementHeight / 2 + 'px';
  
        // Update centerLeft for the next element (cycling)
        if (index === elements.length - 1) {
          // For the last element, set centerLeft to the position of the first element
          const firstElementWidth = elements[0].offsetWidth;
          centerLeft = centerLeft - elementWidth + firstElementWidth;
        } else {
          centerLeft -= elementWidth; // Adjust horizontally for the next element
        }
      }
    });
  }
  ...
  // Example usage:
  const elementIds = ['firstElementId', 'secondElementId', 'thirdElementId']; // Replace with your actual element IDs
  shiftElementsToCenter(elementIds);
