...
const ExperienceCarousel = () => {
  ...
  function shiftArrayLeft(arr) {
    if (arr.length <= 1) {
      // Nothing to shift for arrays with 0 or 1 element
      return arr;
    }
    
    const firstElement = arr.shift(); // Remove and store the first element
    arr.push(firstElement); // Add the stored element to the end of the array
    return arr;
  }
  
  // Example usage:
  const myArray = [0, 1, 2];
  shiftArrayLeft(myArray); // Shifts the elements to the left
  console.log(myArray)
