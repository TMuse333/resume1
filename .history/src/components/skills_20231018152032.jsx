    ...
    const handleScroll = () => {
      ...
      //    console.log(`Skill ${index} - top: ${top}, window.innerHeight - threshold: ${window.innerHeight - threshold}, isInView: ${isInView}`);
          ...
          setSkillsInView((prevSkillsInView) => {
            ...
            newSkillsInView[index] = isInView || prevSkillsInView[index];
            console.log(skillsInView) // Update the state to true if already in view
    ...
    <section className="skills-section">
      ...
      <div className="skills-container ">
        {attributes.map((skill, index) => (
         ...
         >
          <p>{skill.name}</p>
          <p>{isSkillDescriptionVisible[index] ? skill.description : null}</p>
         </motion.div>
        ))}
      </div>
    </section>
