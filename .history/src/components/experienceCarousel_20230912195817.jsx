  ...
  // Call the function to get the widths of the elements
   ...
   function shiftLeft(elementIds, counter) {
    ...
    if (elements.length >= 2) {
      ...
      // Remove the last element from its current position and insert it at the beginning
      parentElement.insertBefore(lastElement, firstElement);
  
      // Apply the same translation for all elements using the calculated distance
      elements.forEach((element, index) => {
        const translateValue = -distance; // Set the translation
        element.style.transform = `translateX(${translateValue}px)`;
  
        // Set the left position to match the current translated position
        const transformMatrix = getComputedStyle(element).transform;
        const transformValues = new WebKitCSSMatrix(transformMatrix);
        const currentTranslateX = transformValues.m41; // Get the X translation component
        element.style.left = currentTranslateX + "px";
      });
    }
  }
