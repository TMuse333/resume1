...
const ExperienceCarousel = () => {
  ...
  const handleNext = () => {
    ...
    setShiftAmount(shiftAmount + 30);
    setRightClicked(ture) // Increase shift amount by 30% with each click
  };

  const handlePrev = () => {
    const prevIndex = (activeIndex - 1 + experiences.length) % experiences.length;
    setActiveIndex(prevIndex);
    setShiftAmount(shiftAmount - 30); // Decrease shift amount by 30% with each click
  };

  const customVariant = (index) => ({
    hidden: {
      x: `${50 + index * 10 + shiftAmount}%`, // Calculate initial x position with shift
    },
    visible: {
      x: rightClicked ? `${50 + index * 10 + shiftAmount + 20}%` : null, // Shift by 20%
      transition: {
        duration: 0.5,
      },
    },
